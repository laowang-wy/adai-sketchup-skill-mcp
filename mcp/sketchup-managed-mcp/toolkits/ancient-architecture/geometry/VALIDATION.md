# 公共几何契约（发行版 0.5.24 / 工具 0.4.4 / 核心 0.1.1）

契约沿用 0.5.6 代码基线；工具与几何核心使用独立版本号。

入口 sketchup_ancient_tool(action=compile,family=geometry,parameters=...)。parameters包含project_id、phase、source_evidence、task_contract及parts（1..100个唯一semantic_id）。正式阶段为massing、roof_profile、archetypes、primary_corrections、facade_detail；refinement是项目mode，不是phase。massing需要projection_subjects，archetypes需要archetypes及至少两个visible_detail_systems，primary_corrections需要correction_targets且task_contract.mode必须为refinement。正式适配器只生成受管登记调用；真机执行、读回和视觉验收仍需在SketchUp中完成。须匹配真实当前context，roof_profile另需roof_control_contract。旧roof/source/bearing/eave/measured保持test/massing门禁。

E12 提供 `roof_recipe` v4 路线：配方只允许当前 `v4/contract.py` 声明的控制字段和有限 preset，离线编译器生成一个语义 ID 为 `roof_shell` 的单一受管 shell，并在 manifest 中记录 `generator_recipe`（版本、源文件哈希、preset 哈希）。它是可复现的输入边界和生产适配器入口，不代表已写入 SketchUp，也不代表屋面细部、瓦件或视觉比例已验收；真实 ComponentDefinition/Instance 登记、读回和审查仍需后续真机阶段。
