...
2. 建模前只调用一次 `action=list`，按来源选定明确 `pack_id`（图片一致性底座是 Skill 内置章节；CAD 用 CAD 包；古建用指定的中式古建包），再以 `pack_ids` 限定 `action=match` → `action=read`。每包只读当前问题的 1—2 节，不把无关经验包整包塞进上下文。经验包用于方法选择和纠错；读取失败记录 `not_checked`，不得伪称已采用，但不把包读取结果变成阻塞主形的流程门槛。
