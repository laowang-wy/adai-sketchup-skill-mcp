...
0.5.26 分为独立 Skill 和 MCP 两个 ZIP。先用宿主资产管理分别导入两个包，并为当前 Agent/专家启用这份 Skill 与 MCP。仅导入 Skill 不会自动注册 MCP 工具，仅解压 MCP 也不会让所有 Agent 自动可见。MCP 的 command 为 node，args 为解压后 launch.cjs 的绝对路径。二者应来自同一发行版本。
