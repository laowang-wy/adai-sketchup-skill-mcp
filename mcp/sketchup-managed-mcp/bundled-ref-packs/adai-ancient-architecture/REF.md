...
所有生成器分别维护参数范围、离线结果、SU记录和外观状态；一种通过不推广至其他类型、参数或版本。上游歇山detailed样板记录封山、六交点与四翼角切瓦；盔顶为带来源推定的候选，非实测还原；完整檐口135点来自A轮，B轮平滑取证未完成。0.5.6 已完成 SU2019 六类默认屋壳与庑殿瓦脊样板读回；完整铺瓦和历史还原仍需当前参数实测。
...
## [massing|roof_profile|facade_detail|finish] 公共几何守卫与源缺陷边界

运行包0.4.4的family=geometry使用公共几何核心0.1.1及正式适配器0.2.0。它接收显式语义构件、来源、project_id、task_contract、phase，可用于 massing/roof_profile/archetypes/primary_corrections/facade_detail；refinement是primary_corrections的mode，不是phase。massing、archetypes、visible_detail_systems和correction_targets必须按契约登记；生成的是受管登记调用，真机执行、读回和视觉验收仍需在SketchUp完成。其它古建生成器仍遵守test/massing诊断限制。先读随工具 geometry/VALIDATION.md 与 Skill references/geometry-guard.md。
曲面使用三角网；未知洞口、局部反面、非流形边立即失败并回滚。仅明确开放片及预期边界、厚度与偏移方向可加厚。逐闭合分量判断外向。自交扫检不是通用CAD布尔证明。
歇山下壳采用明确的内收斜封边，保留上部离散屋面。瓦片从宿主定位实际三角法线；折线处按稳定cell编号选择单侧法线。竖直接触样板只证明声明的竖直偏移。
corridor-corner 的 G0018 边[0,2]、corridor-xieshan 的 G0002 边[92,93]目前编译拒绝 SOURCE_NONMANIFOLD_EDGE；源网络只读保留。歇山可改用通用si_shan壳并明确非源保真；转角应重建有来源的显式构件样板，不能将正身旋转冒充。
新鲜geometry_readback自动生成ID映射、测量、依赖和审查草稿。草稿视觉unverified，必须真实看图。重建重新读PID与测量。固定五视图排除锚点，最多8个语义近景。已接受上游错误用revise_from失效目标与下游。
