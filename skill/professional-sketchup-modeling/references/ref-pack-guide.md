...
1. 建模前：`action=list` → `action=match topics=... stages=当前阶段 project_id=...`
2. 只 `action=read` 匹配到的章节；不要整包读取，也不要凭记忆复述包内容。
3. 匹配结果里的 `packs_used`（版本 + 内容指纹）要写进本次工作记录。
4. 冲突必须交给用户；经验包不能改事务、证据、审查、保存策略。
5. 交付说明要写明本次用了哪些包与版本；没用包就写明原因。
