...
每轮实际核对参考图和能暴露当前问题的对应视图；整体成果至少覆盖一个整体透视和适用的正/侧/平/底或近景，不规定固定张数。优先看 review sheet，缺图或细节不清再打开对应原图。最多抓三个主要缺陷并比较是否改善；连续两轮无改善就回查形制、比例、拓扑、坐标或接口，改变方法，关键主形错误不能用细节掩盖。
